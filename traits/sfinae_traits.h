...
namespace impl::traits{
    ...
    struct has_size_type<T, void_t<typename remove_ref_t<T>::size_type> > : true_type {};
    ...
    template<typename T>
    struct has_first<T, void_t<decltype(declval<T>().first)> > : true_type{};

    template<typename T>
    constexpr inline bool has_first_v = has_first<T>::value;

    template<typename T, typename = void_t<T>>
    struct is_default_constructable : false_type{};

    template<typename T>
    //let it be like in std, don't decay ref to type, so applied to ref = false
    struct is_default_constructable<T, void_t<decltype(T())>> : true_type{};

    template<>
    struct is_default_constructable<void> : false_type{};
    ...
    struct is_convertible{
        ...
        struct is_convertible_helper<From, void_t<decltype(helper(declval<From>()))>> : true_type{};
